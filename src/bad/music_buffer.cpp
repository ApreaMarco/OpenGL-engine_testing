...
void MusicBuffer::UpdateBufferStream()
{
	...
	if (alGetError() != AL_NO_ERROR)
	{
		throw("error checking sound source state");
	}
	...
	while (processed > 0)
	{
		...
		if (alGetError() != AL_NO_ERROR)
		{
			throw("error buffering sound data");
		}
	}
	...
	if (state != AL_PLAYING && state != AL_PAUSED)
	{
		...
		if (alGetError() != AL_NO_ERROR)
		{
			throw("error restarting sound playback");
		}
	}
...
MusicBuffer::MusicBuffer(const char* filename)
{
	...
	if (!p_SndFile)
	{
		throw("could not open sound file -- check path");
	}
